...
## Version: 2.0
##
## REACTIVATE.py - Generate data for calculating Last_Order_Date for each customer and list.
## 
##
## Usage Example:
## python REACTIVATE.py 31122017-extract_orders.csv ALL all_cust_bhv_lod_aa.csv
## python REACTIVATE.py cust-react-dev.csv ALL all_cust_bhv_lod_aa.csv
##
## Version History:
## 17-Jan-2017      Start - define usage and command line. Complete module, basic data read in skelaton.
## 19-Jan-2017      Putting source in data structure skeleton.
## 22-Jan-2017      Completed calculate Last Order Days (LOD) data element.
## 23-Jan-2017      Completed output data file implemetation.
## 
######################################################################################################################
...
if (len(sys.argv) < 2):
    print ">> Usage: REACTIVATE <sale-data-source_csv> <output_list_csv>"
    print ">> Type 'REACTIVATE help' to understand the program usage."
    ...
    ## Real operation start.
        ...
        ## Produce Last Order Days based on Customer-Level structure.
        ## Taking key same as Customer-level key.
        dict_cust_lod = {}        
        produce_lod(dict_cust, dict_cust_lod)
        if(DF): pprint (dict_cust_lod)
        print("Completed processing CUSTOMER-Level info and calculating Last Order Days related info ...\n")

        ## Generate output file.
        rpt_fn = output_file_csv
        lst_output = process_dict_lod_output(dict_cust, dict_cust_lod)
        if(DF): pprint(lst_output)

        save_list_to_file(lst_output, rpt_fn)
        print "Completed saving file: %s ...\n" % output_file_csv
